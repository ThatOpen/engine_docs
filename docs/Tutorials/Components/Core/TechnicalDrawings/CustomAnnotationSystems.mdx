<div style={{position: "relative"}}><iframe src="https://thatopen.github.io/engine_components/examples/TechnicalDrawings/CustomAnnotationSystems"></iframe><button class="full-screen-btn" onClick={() => window.open("https://thatopen.github.io/engine_components/examples/TechnicalDrawings/CustomAnnotationSystems")} >Go Full Screen</button></div>

:::info Source
Copying and pasting? We've got you covered! You can find the full source code of this tutorial [here](https://github.com/ThatOpen/engine_components/blob/main/packages/core/src/drawings/TechnicalDrawings/examples/CustomAnnotationSystems/example.ts).
:::
...
interface CrossMarkerStyle extends OBC.BaseAnnotationStyle {
  /** Half-length of each bar of the cross, in drawing local units. lineSize: number;
}
