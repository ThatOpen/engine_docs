<div style={{position: "relative"}}><iframe src="https://thatopen.github.io/engine_fragment/examples/FragmentsModels"></iframe><button class="full-screen-btn" onClick={() => window.open("https://thatopen.github.io/engine_fragment/examples/FragmentsModels")} >Go Full Screen</button></div>

:::info Source
Copying and pasting? We've got you covered! You can find the full source code of this tutorial [here](https://github.com/ThatOpen/engine_fragment/blob/main/packages/fragments/src/FragmentsModels/example.ts).
:::
  ...
  (_) => {
    ...
    const onLoadModel = async ({ target }: { target: BUI.Button }) => {
      ...
      if (ids.includes(id)) {
        ...
        await loadFragmentFile(`https://thatopen.github.io/engine_fragment/resources/frags/${id}.frag`, id);
      }
